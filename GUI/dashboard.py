...
class _DashBoardMain(QWidget):
    ...
    def customKeyReleaseEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_I and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_AEB_state(0)    
    ...
    def keyAction(self):
        ...
        #     #AEB
        #     if self.keys_[Qt.Key.Key_I]:
        #         self.dash_board_design_widget.set_AEB_state(1)
        #     if self.keys_[Qt.Key.Key_I]:
        #         self.dash_board_design_widget.set_AEB_state(1)    
...
class _DashBoardContolsDesign(QWidget):
    ...
    def AEB_properties(self):
        self.AEB_default_state = 0
        self.AEB_state = 0
        self.AEB_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.HeavyRain))
    ...
    def set_AEB_state(self, val):
        self.AEB_default_state = val
        self.AEB_state = val
        self.repaint()
    ...
    def AEB_painting(self, painter: QPainter):
        # setting AEB font
        AEB_font = QFont("Consolas", 0, 0, True)
        AEB_font.setPixelSize(round(self.width()*0.050))
        AEB_fm = QFontMetrics(AEB_font)
        AEB_rect = AEB_fm.boundingRect("AEB") ##AEB
        painter.setFont(AEB_font)
#####
...
###3
        AEB_rect.moveTo(self.rect().center()+QPointF(self.rect().width()*0.3,  -self.rect().height()*0.008).toPoint())

        painter.setPen(QPen(self.AEB_color_lst[self.AEB_state], round(self.width()*0.0025)))
        painter.drawText(AEB_rect, Qt.AlignmentFlag.AlignCenter, "AEB")
...
class _DashBoardControls(QObject):
    ...
    def startup_values_setter(self):
        ...
        self.dash_board.dash_board_design_widget.set_AEB_state(self.AEB_state)
    ...
    def all_connector(self):
        ...
        self.AEB_sig.connect(self.dash_board.dash_board_design_widget.set_AEB_state)
...
class TriggerAction():
    ...
    def LKW_off(self):
        ...
        self.__dbc.LKW_off()
    def AEB_off(self):
        """To indicate AEB is off"""
        self.__dbc.AEB_off()
    def AEB_on(self):
        """To indicate AEB is on"""
        self.__dbc.AEB_on()        
